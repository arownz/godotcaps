...
This is a Godot 4.4.1 2D web-based educational gamification designed for dyslexic learners with **two distinct core modes**:

1. **Journey Mode**: RPG-style auto turn base battles with STT/Whiteboard word challenges, dungeon progression, and player stats
2. **Module Mode**: Direct educational activities (Phonics, Flip Quiz, Read-Aloud, etc.) with Firebase progress tracking
...
- Phonics (letters/sight words), Flip Quiz, Read-Aloud, Chunked Reading, Syllable Building
...
- `/Scripts/Phonics*.gd`: Phonics module implementations
- `/Scripts/FlipQuiz*.gd`: Flip quiz implementations
- `/Scripts/ModuleScene.gd`: Main module selection
...
- Check ModuleProgress debug logs in Godot output console
...
- Progress calculations verified: phonics = (letters + sight words) / 46 \* 100
