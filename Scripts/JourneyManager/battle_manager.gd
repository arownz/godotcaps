...
func player_attack():
	...
	# Create damage callback that will be triggered at impact moment
	var apply_damage = func():
		var damage = battle_scene.player_manager.player_damage
		emit_signal("player_attack_performed", damage)
		print("BattleManager: Player damage applied at impact: ", damage)
	
	# NOW play attack animation with timed damage callback
	battle_scene.player_manager.perform_auto_attack(apply_damage)
	
	# Wait for attack animation to finish before returning
	if player_sprite and player_sprite.sprite_frames:
		# Check if the current animation will actually finish (sprite_frames exists)
		if player_sprite.is_playing():
			await player_sprite.animation_finished
		else:
			# Fallback if animation isn't playing
...
func enemy_attack():
	...
	# NOW play attack animation
	var animation_played = false
	if enemy_sprite and enemy_sprite.sprite_frames:
		# Check if auto_attack animation exists
		if enemy_sprite.sprite_frames.has_animation("auto_attack"):
			enemy_sprite.play("auto_attack")
			animation_played = true
			print("BattleManager: Playing enemy auto_attack animation")
		else:
			print("BattleManager: Warning - auto_attack animation not found, using fallback timing")
	
	# Create timer to sync damage with impact moment (adjusted for 12 FPS)
	var delay_timer = battle_scene.create_tween()
	delay_timer.tween_interval(0.25) # Impact at ~3rd frame of animation at 12 FPS
	await delay_timer.finished
	
	# Play enemy auto-attack SFX at impact moment
	if battle_scene.enemy_manager and battle_scene.enemy_manager.has_method("play_enemy_sfx"):
		battle_scene.enemy_manager.play_enemy_sfx("enemy_autoattack")
	
	# Apply damage at the exact moment of impact
	var damage = battle_scene.enemy_manager.enemy_damage
	emit_signal("enemy_attack_performed", damage)
	print("BattleManager: Enemy damage applied at impact: ", damage)
	
	# Wait for attack animation to finish OR use fallback timing
	if animation_played:
