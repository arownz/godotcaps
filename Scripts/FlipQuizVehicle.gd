...
func _on_hear_button_pressed():
	...
	if current_vehicle_index < selected_vehicles.size():
		...
		var sound_node = get_node_or_null(vehicle.sound_node)
		if sound_node:
			sound_node.play()
			
			# Wait a moment, then speak the vehicle name clearly (secondary hint)
			await get_tree().create_timer(2.5).timeout
			if tts and hear_button and hear_button.text == "Stop": # Check if still Stop mode
...
func _on_hear_tts_finished():
	...
	if hear_button:
		...
		if instruction_label:
			instruction_label.add_theme_color_override("default_color", Color(1.0, 0.6, 0.0, 1))
